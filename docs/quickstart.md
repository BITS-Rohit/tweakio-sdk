...
```python
import asyncio
from src.BrowserManager import ProfileManager, BrowserConfig, CamoufoxBrowser, Platform
...
async def main():
    ...
    browser = CamoufoxBrowser(config=config, profileInfo=profile, log=logger)
