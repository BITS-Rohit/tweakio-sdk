...
def test_get_fg_loads_existing(browserforge, mock_fingerprint, tmp_path):
    ...
    with patch("src.BrowserManager.browserforge_manager.os.stat") as mock_stat:
        mock_stat.return_value.st_size = 100
        with patch(
            "src.BrowserManager.browserforge_manager.pickle.load", return_value=mock_fingerprint
        ):
            with patch("src.BrowserManager.browserforge_manager.open", mock_open()):
...
def test_get_fg_generates_new(browserforge, mock_fingerprint, tmp_path, mock_logger):
    ...
    with patch.object(browserforge, "_gen_fg", return_value=mock_fingerprint):
        with patch("src.BrowserManager.browserforge_manager.pickle.dump") as mock_dump:
            with patch("src.BrowserManager.browserforge_manager.open", mock_open()):
                result = browserforge.get_fg(fg_path)
...
def test_gen_fg_success(browserforge, mock_fingerprint, mock_logger):
    """Test _gen_fg generates valid fingerprint matching screen size."""
    with patch(
...
def test_gen_fg_retries_on_mismatch(browserforge, mock_logger):
    """Test _gen_fg retries if fingerprint screen doesn't match."""
...
def test_gen_fg_max_attempts(browserforge, mock_logger):
    """Test _gen_fg stops after 10 attempts and returns last fingerprint."""
    bad_fg = Mock(spec=Fingerprint)
    bad_fg.screen = Mock(width=800, height=600)
...
def test_gen_fg_invalid_screen_size(browserforge):
    """Test _gen_fg raises error for invalid screen dimensions."""
    with patch(
        "src.BrowserManager.browserforge_manager.BrowserForgeCompatible.get_screen_size",
        return_value=(0, 0),
    ):
        with pytest.raises(BrowserException, match="Invalid real screen dimensions"):
            browserforge._gen_fg()
